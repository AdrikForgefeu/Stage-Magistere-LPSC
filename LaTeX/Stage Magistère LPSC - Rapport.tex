...
\end{equation}
où $f(x) = x\coth\!\frac{x}{2}-4$ est la dépendance vis-à-vis de la fréquence, sans prendre en compte les corrections relativistes. $T_\text{CMB}$ est la température du CMB, $k_B$ la constante de Boltzmann. Pour plus de détail sur le paramètre Compton $y$, se référer à l'appendice \ref{AppendixA}.
...
\section{Conclusion}
\label{Section4}
Lorsque le rapport $\nicefrac{M_{500}}{M_\text{c}}$ diminue, cela signifie que la densité du gaz est faible au centre du halo et une absence de rétroaction dû aux vents stellaires. Dans ce cas, la galaxie n'a pas d'AGN. De plus, la variation $\Delta T_\text{kSZ}$ est moins importante. L'absence de noyau actif  possède donc impact indirect sur l'intensité de la fluctuation de température aux abords du halo.\\ 
